...
export const getStateList = (action$) =>
  action$.pipe(
    ofType("GET_ALL_STATE_REQUEST"),
    mergeMap((action) =>
      from(API.post("/settings/getAllState", action.payload)).pipe(
        mergeMap((response) => {
...
export const addNewState = (action$) =>
  action$.pipe(
    ofType("ADD_NEW_STATE_REQUEST"),
    mergeMap((action) =>
      from(API.post("/settings/createUpdateState", action.payload)).pipe(
        mergeMap((response) => {
          console.log(response);
          return of({
...
export const deleteState = (action$) =>
  action$.pipe(
    ofType("DELETE_STATE_REQUEST"),
    mergeMap((action) =>
      from(API.post("/settings/deleteState", action.payload)).pipe(
        mergeMap((response) => {
          console.log(response);
          return of({
...
export const editState = (action$) =>
  action$.pipe(
    ofType("EDIT_STATE_REQUEST"),
    mergeMap((action) =>
      from(API.get(`/settings/editState/${action.payload}`)).pipe(
        mergeMap((response) => {
          console.log(response);
          return of({
