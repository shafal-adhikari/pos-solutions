...
function AddDocumentType({ isOpen, setIsOpen }) {
  ...
  useEffect(() => {
    if (searchKeyword) {
      const searchTimeout = setTimeout(async () => {
        dispatch({
          type: "GET_ALL_DOCUMENT_TYPE_REQUEST",
          payload: {
            Page: 1,
            PageSize: 5,
            ExternalFilter: {
  ...
  useEffect(() => {
    if (totalDocumentType == 0 && currentPagination > 1) {
  ...
  const editDocumentType = (id) => {
    dispatch({
      type: "EDIT_DOCUMENT_TYPE_REQUEST",
      payload: id,
    });
  };
  const bulkDeleteHandler = (e) => {
    e.preventDefault();
    dispatch({
      type: "DELETE_DOCUMENT_TYPE_REQUEST",
      payload: [...bulkSelected],
    });
  };
  const deleteDocumentTypeHandler = (id, name) => {
    dispatch({
      type: "DELETE_DOCUMENT_TYPE_REQUEST",
      payload: [
        {
  ...
  const addDocumentTypeHandler = (values) => {
    dispatch({
  ...
  return (
    ...
    >
      ...
      <Modal.Body className="modal-body">
        <Form
          form={form}
          onFinish={addDocumentTypeHandler}
          initialValues={{
            isActive: true,
          }}
        >
          ...
          <div className="card-body">
            <div className="table-body">
              ...
              {allLoading ? (
                ...
                <div className="table-responsive">
                  <table className="table table-hover  align-middle table-nowrap mb-0">
                        ...
                        <th>
                          <div className="form-check">
                            <input
                              ...
                              type="checkbox"
                              checked={
                                allDocumentType?.length == bulkSelected.length
                              }
                              value={
                                allDocumentType?.length == bulkSelected.length
                              }
                    ...
                    <tbody>
                      {allDocumentType?.length > 0 ? (
                        allDocumentType?.map((leaveType) => {
                          return (
                            <tr key={leaveType.id}>
                              <td>
                                <div className="form-check">
              ...
              <div className="company_footer d-flex justify-content-end mt-3">
                <Pagination
                  total={totalDocumentType}
                  showTotal={(total, range) =>
                    `${allDocumentType ? allDocumentType?.length : 0} out of ${
                      total ? total : 0
                    } items`
                  }
                  pageSize={5}
