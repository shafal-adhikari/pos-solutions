...
function AddYear({ isOpen, setIsOpen }) {
  ...
  useEffect(() => {
    if (totalYear == 0 && currentPagination > 1) {
  ...
  const bulkDeleteHandler = (e) => {
    e.preventDefault();
    dispatch({
      type: "DELETE_YEAR_REQUEST",
      payload: [...bulkSelected],
    });
  };
  const deleteYearHandler = (id, name) => {
    dispatch({
      type: "DELETE_YEAR_REQUEST",
      payload: [
        {
  ...
  const addYearHandler = (values) => {
    dispatch({
      type: "ADD_YEAR_REQUEST",
      payload: [
        {
          ...values,
                        ...
                        <th>
                          <div className="form-check">
                            <input
                              ...
                              type="checkbox"
                              checked={allYear?.length == bulkSelected.length}
                              value={allYear?.length == bulkSelected.length}
                              onChange={(e) => {
                                if (e.target.checked) {
                                  setBulkSelected(
                                    allYear?.map((category) => {
                                      return {
                                        Id: category.id,
                                        Name: category.name,
                                      };
              ...
              <div className="company_footer d-flex justify-content-end mt-3">
                <Pagination
                  ...
                  showTotal={(total, range) =>
                    `${allYear ? allYear?.length : 0} out of ${
                      total ? total : 0
                    } items`
                  }
                  pageSize={5}
