...
import { Button, Form, Popconfirm, Select, Spin } from "antd";
...
function AddPayScaleLevel({ isOpen, setIsOpen }) {
  ...
  useEffect(() => {
    if (searchKeyword) {
      const searchTimeout = setTimeout(async () => {
        dispatch({
          type: "GET_ALL_PAY_SCALE_REQUEST",
          payload: {
            Page: 1,
            PageSize: 5,
            ExternalFilter: {
  ...
  useEffect(() => {
    if (totalPayScaleLevel == 0 && currentPagination > 1) {
  ...
  const editPayScaleLevel = (id) => {
    dispatch({
      type: "EDIT_PAY_SCALE_REQUEST",
      payload: id,
    });
  };
  ...
  const addLeaveTypeHandler = (values) => {
    dispatch({
      type: "ADD_NEW_PAY_SCALE_REQUEST",
      payload: [
        {
          ...values,
                        ...
                        <div className="form-group">
                          ...
                          >
                            <Input
                              ...
                              placeholder="Pay Scale Level Name"
                            />
                          </Form.Item>
                        </div>
                      </div>
          ...
          <div className="card-body">
            <div className="table-body">
              ...
              {isLoading ? (
                ...
                <div className="table-responsive">
                  <table className="table table-hover  align-middle table-nowrap mb-0">
                        ...
                        <th>
                          <div className="form-check">
                            <input
                              ...
                              type="checkbox"
                              checked={
                                allPayScaleLevel?.length == bulkSelected.length
                              }
                              value={
                                allPayScaleLevel?.length == bulkSelected.length
                              }
                    ...
                    <tbody>
                      {allPayScaleLevel?.length > 0 ? (
                        allPayScaleLevel?.map((leaveType) => {
                          return (
                            <tr key={leaveType.id}>
                              <td>
                                <div className="form-check">
                              ...
                              <td>
                                <a
                                  ...
                                  onClick={(e) => {
                                    e.preventDefault();
                                    editPayScaleLevel(leaveType.id);
                                  }}
              ...
              <div className="company_footer d-flex justify-content-end mt-3">
                <Pagination
                  total={totalPayScaleLevel}
                  showTotal={(total, range) =>
                    `${allPayScaleLevel ? allPayScaleLevel.length : 0} out of ${
                      total ? total : 0
                    } items`
                  }
                  pageSize={5}
...
export default AddPayScaleLevel;
